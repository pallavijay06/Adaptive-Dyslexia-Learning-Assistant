...
quiz_payload = json.dumps({'document_id': upload['document_id'], 'num_mcqs': 4, 'num_short_questions': 4}).encode('utf-8')
req = urllib.request.Request(f'{BASE}/quiz/generate', data=quiz_payload, headers={'Content-Type': 'application/json'})
try:
    res = urllib.request.urlopen(req, timeout=120)
    quiz = json.loads(res.read().decode())
    print('QUIZ STATUS', res.status)
    print('QUIZ BODY', json.dumps(quiz)[:4000])
except urllib.error.HTTPError as e:
    print('QUIZ HTTP ERROR', e.code)
    print(e.read().decode())
    raise

chat_payload = json.dumps({'message': 'Explain the main idea of the document in simple words.', 'document_id': upload['document_id']}).encode('utf-8')
